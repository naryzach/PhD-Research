...
# Therefore, the 6 residues to be replaced are at:
# Start index in mature sequence (0-based): 30 - 24 = 6
# End index in mature sequence (0-based, exclusive): 35 - 24 + 1 = 12
...
def generate_fasta_files():
    ...
    # 0. Define all 20 variant sequences provided by the user
    VARIANT_SEQUENCES = [
    ...
    # 1. Fetch all required sequences (only once)
    ...
    # 2. Extract the Mature TIMP3 Sequence (residues 24-211)
    # 0-based index: start at 23, end at 211 (exclusive slice)
    TIMP3_MATURE_WT = timp3_full_seq[23:] 
    
    # Validate the segment we are about to replace (should be 'PCCEVC')
    segment_to_replace = TIMP3_MATURE_WT[REPLACEMENT_START_INDEX : REPLACEMENT_START_INDEX + REPLACEMENT_LENGTH]
    print(f"Wild-type 6-residue segment being replaced (Mature AA 7-12): {segment_to_replace}")
    print(f"Total number of variant sequences to process: {len(VARIANT_SEQUENCES)}\n")
    
    # 3. Iterate through all variants and generate files
    ...
    for i, variant_sequence in enumerate(VARIANT_SEQUENCES):
        ...
        # 4. Construct the Variant TIMP3 Sequence
        TIMP3_VARIANT_SEQ = (
        ...
        # 5. Define sequences for the two AlphaFold runs
        for name, uid in UNIPROT_IDS.items():
            if name == "TIMP3":
                continue
