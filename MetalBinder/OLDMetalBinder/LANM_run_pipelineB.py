        ...
        try:
            ...
            if mpnn_outputs:
                ...
                for loop_info in job['loops']:
                    loop_mask = (mpnn_out.atom_array.res_id >= loop_info['start_res']) & \
                                (mpnn_out.atom_array.res_id <= loop_info['end_res']) & \
                                (mpnn_out.atom_array.atom_name == 'CA')
                    
                    loop_ca_atoms = mpnn_out.atom_array[loop_mask]
                    # Ensure they are sorted by res_id in case they are out of order
                    order = np.argsort(loop_ca_atoms.res_id)
                    loop_res_names = loop_ca_atoms.res_name[order]
                    
                    loop_seq_letters = []
                    for rn in loop_res_names:
                        try:
                            loop_seq_letters.append(ProteinSequence.convert_letter_3to1(rn))
                        except:
                            loop_seq_letters.append("X")
                    loop_sequences.append("".join(loop_seq_letters))
    ...
    for job in rf3_queue:
        ...
        try:
            ...
            if rf3_target_key in rf3_outputs_dict:
                ...
                # --- Per-Loop Metrics ---
                for i, loop_info in enumerate(job['loops']):
                    loop_mask = (bb_generated.res_id >= loop_info['start_res']) & (bb_generated.res_id <= loop_info['end_res'])
                    l_bb_gen = bb_generated[loop_mask]
                    l_bb_ref = bb_refolded_fitted[loop_mask]
                    
                    l_rmsd = rmsd(l_bb_gen, l_bb_ref) if len(l_bb_gen) > 0 else float('nan')
                    l_radius = calculate_binding_radius(rf3_atom_array, loop_info, loop_info['start_res'], loop_info['end_res'])
                    l_seq = job['loop_sequences'][i]
                    
                    print(f">>> {job['design_id']} | Loop {i+1} | Seq: {l_seq} | Loop RMSD: {l_rmsd:.2f} | Radius: {l_radius:.2f} A", flush=True)
