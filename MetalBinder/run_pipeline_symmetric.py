    ...
    for ion in args.ions:
        ...
        for config_idx in range(args.num_configs):
            ...
            for loop_info in sorted_loops:
                ...
                # SYMMETRIC VARIANT (Iteration 14 / Phase B2 experiment): the
                # original range [native-2, native+3] has midpoint native+0.5,
                # a systematic bias toward longer-than-wildtype loops on every
                # design regardless of target ion. Phase B (150 pairwise 4-ion
                # validations) found 15/15 non-La-targeted designs bound La
                # (the largest tested ion, r=1.032A) at least as well as their
                # own native ion -- this symmetric range removes that specific
                # asymmetry as a controlled test of whether it's the cause.
                native_loop_length = end_res - start_res + 1
                min_len = max(1, native_loop_length - 2)
                max_len = native_loop_length + 2
