...
def parse_args():
    ...
    parser.add_argument("--input", type=str, default="../Data/1CLL.cif",
                        help="Path to input CIF structure (default: 1CLL, canonical "
                             "Ca-bound calmodulin -- this is the CaM-backbone variant "
                             "of run_pipeline.py, Iteration 16 / Phase F)")
...
def main():
    ...
    for ion in args.ions:
        ...
        # CaM-backbone variant: 1CLL's native ion is CA, not ND, and its 4
        # EF-hand loop ranges (identified directly from the structure -- Ca
        # ions sit nearest chain-A residues 20, 56, 93, 129) are different
        # residue numbers than 8FNS's. Both differences are isolated to this
        # one call site; everything else in the pipeline is unchanged.
        CAM_EF_RANGES = [(20, 31), (56, 67), (93, 104), (129, 140)]
        atom_array = mutate_metals(base_atom_array.copy(), original_res_name="CA", new_res_name=ion)
        loops = get_ef_hand_loops(atom_array, metal_res_name=ion,
                                   ef_ranges=CAM_EF_RANGES, protein_chain_id="A")
        
        print(f"\nFound {len(loops)} metal binding sites for {ion}.")
            
        # 1CLL also carries an EOH (ethanol, crystallization additive) HETATM
        # not present in 8FNS -- strip it alongside water so it doesn't reach
        # RFd3 as spurious context.
        water_mask = np.isin(atom_array.res_name, ["HOH", "EOH"])
